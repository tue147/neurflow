...
python -m exps.fidelity_of_neuron.Fidelity_using_neuronmct.py --data_dir "$DATA_DIR" --label_list "${LABEL_LIST[@]}" --model "$MODEL" --load_dir "$LOAD_DIR" --list_tau "${TAU_LIST[@]}" --batch_size "$BATCH_SIZE" --device "$DEVICE"