...
from utils import split_model, get_sub_model, batch_inference, get_conditional_modules, load_model_data
...
def pruned_probing(
    ...
    for layer in range(start, end+1): 
        ...
        if not random_mask:
            ...
            indices = random.sample(range(0, intermediate.shape[1]), len(indices))
            mask[indices] = 1 if reverse else 0
