...
def generate_unique_lists(N, M, list_lengths):
    """
    Generates N unique lists with RANDOM LENGTH sampled from list_lengths.

    Args:
        N (int): Number of lists to generate.
        M (int): Maximum value of elements in the lists.
        list_lengths (list or range): Possible lengths of each list.

    Returns:
        list: A list of numpy arrays, each representing a unique list.
    """
    unique_lists = []
    for _ in range(N):
        list_length = random.choice(list_lengths)  # Randomly choose a list length
        if list_length > M + 1:
            raise ValueError("list_length must be less than or equal to M + 1 to ensure all elements in the list are unique")
...
for i, test_layer in enumerate(test_layers):
    correlations.append({"True": [], "False": []})
    ...
    for target_label in label_list:
        ...
        M = num_node - 1
        list_masking = generate_unique_lists(N, M, list_lengths)
        scores = np.array(
            calculate_score_multi_masking(
                netB, 
                intermediate, 
                list_masking, 
                device, 
        ...
        for indices in list_masking:
            multi_importances.append(np.sum(importance[indices])) # sum: avg = false
            
        multi_importances = np.array(multi_importances)
        correlation_matrix = np.nan_to_num(np.corrcoef(scores, multi_importances), nan=0.0)
        correlation_coefficient = correlation_matrix[0, 1]
        
        print("Layer:", all_layers[test_layer-1], " Avg: False", " Correlation:", correlation_coefficient)
        correlations[i]["False"] = correlation_coefficient
        
        # MEAN
        multi_importances = []
        for indices in list_masking:
            multi_importances.append(np.mean(importance[indices])) # mean: avg = true
        
        multi_importances = np.array(multi_importances)
        correlation_matrix = np.nan_to_num(np.corrcoef(scores, multi_importances), nan=0.0)
        correlation_coefficient = correlation_matrix[0, 1]
        
        print("Layer:", all_layers[test_layer-1], " Avg: True", " Correlation:", correlation_coefficient)
        correlations[i]["True"] = correlation_coefficient
                
        correlations[i] = {key: np.mean(values) for key, values in correlations[i].items()}
        
    torch.save(correlations, output_dir + f"correlations_{method_name}_{model_name}_compare_sum_and_mean.pth")
