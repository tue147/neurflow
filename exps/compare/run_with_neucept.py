...
parser.add_argument('--batch_size_neucept', type=int, default=50, help='Batch size for the neucept')
...
if model_name == "resnet50":
    ...
    layers = ["fc", "layer4.2", "layer4.1"]  # first few layers because of long running time of Knockoff
    conditional_modules = get_conditional_modules(model_name)
elif model_name == "googlenet":
    model = models.googlenet(pretrained=True).eval()
    layers = ["fc", "inception5b", "inception5a"]
    conditional_modules = get_conditional_modules(model_name)
elif model_name == "alexnet":
    model = models.alexnet(pretrained=True).eval()
    layers = ["classifier.6", "classifier.5", "classifier.2"]
    conditional_modules = get_conditional_modules(model_name)
else:
...
from NeurFlow.exps.compare.Framework_with_neucept import Framework
...
for layer in range(end, start-1, -1):
    ...
    FW.fit(
        ...
        batch_size_ig=args.batch_size_neucept,
