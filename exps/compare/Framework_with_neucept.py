...
class Framework():
    ...
    def _get_knockoff(
        ...
        '''
        Get Knockoff scores for each lower layer neuron.
        '''
    ...
    def _find_critical_neurons(
        ...
        for neu, critical_neu in tqdm(self.layers[layer].items()):
            ...
            list_scores = self._get_knockoff(
                prev_layer_activation[top_imgs], neu, torch.transpose(activations, 0, 1)[top_imgs, :]
            )
            _, indices = torch.topk(list_scores, max_top_neurons)
